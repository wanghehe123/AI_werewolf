...
def test_initialize_game_node_can_fix_human_role():
    board = default_boards()[0]
    agents = default_agents()[:5]

    state = initialize_game_node(board=board, human_player_id="human", agents=agents, seed=1, human_role_key="seer")

    assert state.player_by_id("human").role_key == "seer"
    assert len([player for player in state.players if player.role_key == "seer"]) == 1
