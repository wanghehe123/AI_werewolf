...
## 专业技能 - Python 部分补充
熟悉 Python 语言及异步编程（asyncio/await），熟悉 FastAPI 框架及 WebSocket/SSE 实时通信。
熟悉 LangChain、LangGraph 等 LLM 应用框架，具备多 Agent 编排与 Prompt Engineering 实践经验。
熟悉 Pydantic 数据校验与序列化，具备 pytest 单元测试与异步测试编写经验。
